...
@MainActor
final class PhoneReportsViewModel: ObservableObject {
  @Published var isLoading = false
  @Published var error: String?

  @Published var reports: [PhoneReport] = []
  ...
  let reportCreateSubject = PassthroughSubject<PhoneReportDetails, Never>()
  let reportUpdateSubject = PassthroughSubject<PhoneReportDetails, Never>()
  ...
  private func addReport(_ report: PhoneReportDetails) {
    reports.append(PhoneReport.from(details: report))
    totalReports += 1
  }

  private func updateReport(_ updatedReport: PhoneReportDetails) {
    if let index = reports.firstIndex(where: { $0.phoneNumber == updatedReport.phoneNumber }) {
  ...
  func loadReports(reset: Bool = false) async {
    ...
    do {
      let response: PhoneReportResponse = try await service.searchReports(
        page: 1,
        pageSize: pageSize,
        query: searchText
      )
  ...
  func deleteReport(_ report: PhoneReport) async {
    error = nil

    guard let index = reports.firstIndex(where: { $0.phoneNumber == report.phoneNumber }) else {
    ...
    do {
      let success = try await service.deleteReportBy(phoneNumber: report.phoneNumber)
      guard success else {
